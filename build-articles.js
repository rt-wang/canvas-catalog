/* Builds a reading page per article.
 *
 *   site/articles/src/<slug>.txt  ->  site/articles/<slug>.html
 *
 * The list lives in articles.json; the author name comes from site/content.js,
 * so there is only ever one place to change it. Run: npm run build
 */
const fs = require("fs");
const path = require("path");

const ROOT = path.join(__dirname, "site", "articles");
const SRC = path.join(ROOT, "src");
const MANIFEST = require("./articles.json");
const { SITE } = require("./site/content.js");

const esc = s => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
const isSection = l => /^([一二三四五六七八九十]+|\d{1,2})$/.test(l.trim());
...
/* CJK has no spaces, so counting words there is meaningless — count characters. */
...
:root{
  ...
  --ink-soft:#6E6F6A;
  --rule:#C6C5BF;
  --accent:${SITE.accent || "#2320E8"};
  --display:"Archivo","Helvetica Neue",Arial,sans-serif;
  --body:"Newsreader",Georgia,"Times New Roman",serif;
  --cjk:"Newsreader","Songti SC","Noto Serif SC",serif;
...
body{
  ...
  font-family:${cjk ? "var(--cjk)" : "var(--body)"};
...
header.piece{
  ...
  border-bottom:1px solid var(--rule);margin-bottom:clamp(28px,4vw,52px);
}
.eyebrow{color:var(--ink-soft);margin-bottom:clamp(16px,2.4vw,26px)}
h1{
  margin:0;font-family:var(--display);
  font-variation-settings:"wdth" 112,"wght" 840;
  font-size:clamp(32px,6.4vw,78px);line-height:.92;
  letter-spacing:-.02em;text-transform:uppercase;max-width:16ch;
}
.byline{margin-top:clamp(16px,2.2vw,24px);color:var(--ink-soft)}
article{
  max-width:${cjk ? "40em" : "34em"};
  font-size:${cjk ? "clamp(16px,1.35vw,18px)" : "clamp(17px,1.45vw,19.5px)"};
  line-height:${cjk ? "1.95" : "1.62"};
  padding-bottom:clamp(48px,7vw,96px);
}
article p{margin:0 0 1.15em}
${cjk ? "article p{text-align:justify;text-justify:inter-ideograph}\n" : ""}.sec{
  margin:2.4em 0 1.6em;color:var(--ink-soft);
  border-top:1px solid var(--rule);padding-top:1.1em;
  font-size:13px;letter-spacing:.05em;
}
article .sec:first-child{border-top:0;padding-top:0;margin:0 0 1.6em}
footer{
  border-top:1px solid var(--rule);padding:18px var(--gutter) 40px;
  display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap;color:var(--ink-soft);
}
footer a{color:var(--accent);text-decoration:none;border-bottom:1px solid var(--accent);padding-bottom:2px}
...
<header class="topbar">
  <a class="topbar__name" href="../index.html">${esc(SITE.name)}</a>
  <a class="back mono" href="../index.html">← Catalog</a>
</header>

<div class="wrap">
  <header class="piece">
    <div class="eyebrow mono">${esc(a.kind || "Essay")} · ${a.date.slice(0, 4)}</div>
    <h1>${esc(a.title)}</h1>
...
<footer class="mono">
  <span>© ${a.date.slice(0, 4)} ${esc(SITE.name)}</span>
...
for (const a of MANIFEST) {
  ...
  if (!fs.existsSync(src)) {
    console.error(`  ! missing source: site/articles/src/${a.slug}.txt — skipped`);
    continue;
  }
  const text = fs.readFileSync(src, "utf8");
  fs.writeFileSync(path.join(ROOT, a.slug + ".html"), page(a, text));
  console.log(`  ${a.slug.padEnd(28)} ${a.date}  ${measure(text, a.lang)}`);
  built++;
}
console.log(`built ${built} article page${built === 1 ? "" : "s"}`);
